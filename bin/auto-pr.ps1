...
function pull_requests($json, [String] $app, [String] $upstream, [String] $manifest, [String] $commitMessage) {
    ...
    Write-Host "hub ls-remote --exit-code origin $branch" -ForegroundColor Green
    hub ls-remote --exit-code origin $branch
    ...
    if ($Bot) {
        Write-Host "Creating and Pushing update $app ($version) via GitHub GraphQL API ..." -ForegroundColor DarkCyan
        $response = graphql_commit_push -t $TOKEN -RepoNwo $OriginRepoNwo -b $branch -f $manifest `
         -MessageHeadline $CommitMessage `
        ...
        if (!$response.data.createCommitOnBranch.commit.url) {
            error "Commit and push $app ($version) via GitHub GraphQL API failed! (`n'$( $response | ConvertTo-Json -Depth 100 )'`n)"
            execute 'hub reset'
            return
        }
    } else {
...
function graphql_commit_push {
    ...
    param (
        # Remote repository name with owner requested by GitHub GraphQL API
        [String] $RepoNwo,
        # Repository branch name requested by GitHub GraphQL API
        [String] $Branch,
...
if ($Push) {
    ...
    execute "hub pull --rebase --autostash upstream $UpstreamBranch"
    ...
    if ($Push) {
        ...
        if ($status -and $status.StartsWith('M  ') -and $status.EndsWith("$app.json")) {
            if ($Bot) {
                ...
                if ($response.data.createCommitOnBranch.commit.url) {
                    execute "hub fetch origin $OriginBranch"
                } else {
                    error "Commit and push $app ($version) via GitHub GraphQL API failed! (`n'$( $response | ConvertTo-Json -Depth 100 )'`n)"
                }
            } else {
